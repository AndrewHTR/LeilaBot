...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        if data is None:
            raise YTDLError('Não foi possivel achar semelhantes a: `{}`'.format(search))
        ...
        if processed_info is None:
            raise YTDLError('Não foi possivel encontrar: `{}`'.format(webpage_url))
        ...
        if 'entries' not in processed_info:
            ...
            while info is None:
                try:
                    ...
                    raise YTDLError('Não foi possivel conseguir buscas semelhantes a: `{}`'.format(webpage_url))
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Tocando agora:',
                 ...
                 .add_field(name='Link para a música:', value='[Aperte aqui]({0.source.url})'.format(self))
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('Esse comando não pode ser usado pela dm.')
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('Você ou não está em um canal de voz ou não especifiou um canal de voz para que eu entre.')
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            await ctx.send('Você já adicionou um voto.')
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('Você não está conectado a nenhum canal de voz. :rolling_eyes:')
